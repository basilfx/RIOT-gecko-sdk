...
def AdjustMaxValues(curveSegments):
    for i in range(1, len(curveSegments)):
        if curveSegments[i].slope != curveSegments[i-1].slope:
          curveSegments[i].maxValue = max(curveSegments[i].maxValue,int((curveSegments[i-1].intercept - curveSegments[i].intercept)/(curveSegments[i].slope - curveSegments[i-1].slope)))
